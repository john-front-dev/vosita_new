...
import { useRemoveCategory } from '../model/use-category-mutations';
...
export const CategoriesPage = () => {
  ...
  const { pagination, queryParams, searchText, setSearchText, setType, type } = useUrlListState({
    ...
    defaultType: 'os',
    isType: isCategoryType,
  });
  const canCreate = canManage || type === 'tmz' || type === 'mbp';
  const { isFetching, isLoading, records, totalCount } = useCategoriesList({
    limit: queryParams.limit,
    page: queryParams.page,
    searchText: queryParams.searchText,
    type,
  });
  const { isRemoving, remove } = useRemoveCategory(type === 'os' ? 'os' : 'mbp', () =>
    setRemovingRecord(null),
  );
  const columns = useMemo<DataTableProps<CategoryRecord>['columns']>(() => {
    const result: DataTableProps<CategoryRecord>['columns'] =
      type === 'os'
        ? [
            ...
            { accessor: 'tax_group_id', minWidth: '130px', title: 'Налог' },
            {
              ...
              minWidth: '130px',
              renderRowCell: (record) => `${(record as OsCategory).depreciation_rate}%`,
              title: 'Норма',
            },
          ]
        : type === 'tmz'
          ...
          : [
              ...
              { accessor: 'name', minWidth: '300px', title: 'Название' },
              {
                accessor: 'is_destroyable',
                minWidth: '210px',
                renderRowCell: (record) => (
                  <Badge
                    size="m"
                    type="secondary"
                    variant={(record as MbpCategory).is_destroyable ? 'info' : 'neutral'}
                  >
                    {(record as MbpCategory).is_destroyable ? 'Да' : 'Нет'}
                  </Badge>
                ),
                title: 'Подлежит уничтожению',
              },
            ];
        ...
        renderRowCell: (record) => (
          <div className="flex justify-end gap-1" onClick={(event) => event.stopPropagation()}>
            ...
            <Button
              ...
              isIconBtn
              aria-label="Удалить"
              onClick={() => setRemovingRecord(record)}
            >
              <OutlineSystemTrash />
            </Button>
          </div>
    ...
    <section className="flex min-h-[calc(100vh-48px)] flex-1 flex-col gap-5">
      ...
      <Surface className="flex flex-1 flex-col gap-4" p="6" rounded="12">
        ...
        <DataTable
          ...
          isLoading={isLoading}
          onRowClick={
            type === 'tmz' ? (record) => setTmzDetailsId((record as TmzCategory).id) : undefined
          }
          emptyPlaceholder={
            queryParams.searchText
              ? `По поиску «${queryParams.searchText}» ничего не найдено.`
              : 'Список категорий пока пуст.'
          }
      ...
      {isAddOpen &&
        (type === 'tmz' ? (
          <TmzCategoryAddModal isOpen onClose={() => setIsAddOpen(false)} />
        ) : (
          <CategoryFormModal isOpen type={type} record={null} onClose={() => setIsAddOpen(false)} />
        ))}
      {editingRecord && type !== 'tmz' && (
        <CategoryFormModal
          isOpen
          type={type}
      ...
      <TmzCategoryDetailsModal
        categoryId={tmzDetailsId}
        canManage={canManage}
        onClose={() => setTmzDetailsId(null)}
      />
      <ConfirmModal
        isOpen={removingRecord !== null}
        ...
        onConfirm={() => removingRecord && remove(removingRecord.id)}
      />
    </section>
  );
};
